...
def build_general_rag_prompt(query: str, context: str, warning: str = '') -> str:
    ...
    return f"""
            ...
            6. 如果用户问的是概括类问题（如某主题的历史、概况、背景），请先直接回答用户所问对象的核心信息，再适当补充来源说明，避免用细枝末节的信息开场。
            7. 如果参考上下文只是顺带提到问题中的实体，但没有直接讨论用户询问的主题、属性或关系，必须明确拒答，不能拿顺带提及的句子充当答案。
            8. 不要把用户问题中的前提、修饰词、比较关系、因果关系或身份描述直接当作事实复述；只有参考上下文明确支持时，才能写进答案。
...
def build_structured_rag_prompt(query: str, context: str) -> str:
    return f"""
            ...
            3. 根据问题需要输出适当长度的答案。如果问题涉及多个信息点（如列举、角色、包含哪些），请完整列出所有相关内容，不要遗漏。
            4. 不能编造参考资料里没有的事实。
            5. 如果资料不足以直接回答，就明确回答“根据当前检索到的知识库内容，我无法确定。”
            6. 不要把用户问题中的前提、修饰词、比较关系、因果关系或身份描述直接当作事实复述；只有参考资料明确支持时，才能写进答案。
